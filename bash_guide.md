...
<a href = "https://upload.wikimedia.org/wikipedia/commons/2/24/Male_mallard_duck_2.jpg"> <img source = "http://officeducks.com/wp-content/uploads/2015/05/27-degree-Duck.png" > </a> 

## Contents

* [Common Bash Commands](#common-bash-commands)
* [Storage and System Commands](#storage-and-system-commands)
* [Mac-Specific Bash Commands](#mac-specific-bash-commands)
    * [Flags](#flags)
...
* [Navigation and Editing Shortcuts](#navigation-and-editing-shortcuts)
...
* `ln`: create a link to another directory or file (symbolic links only). The `-s` option makes the link *symbolic*, and is distinguishable from the original directory or file. By default, `ln` creates a *hard* link to a file, which is indistinguishable from the original (i.e., changes to one affects all)
...
* `ping`: send, receive and time packages to a specific server. For example `ping -c 5 google.com` sends and receives 5 packets from the nearest google.com server. Then, it produces timing statistics based on that.
...
* `tar`: Tape archive utility, creates and unpacks archive files out of one or many file and supports compression. Example: `tar -xvf log.tar.gz`, which unpacks a compressed `.tar` file (`-x`) with verbose option (`-v`).
...
* `tar`: create a *tape archive* file out of many files for simplified movement of data.
    ...
    * Extract Archive: `tar -xf myArchive.tgz dir1`: extract files from `myArchive` (compressed with `gzip`) and place in dir1.
...
If a `:` precedes the arguments after getopts (e.g., `:a:b:`), then any flag may be entered and caught in the `case` statement with `?)`. For example,
...
All envinronmental variables may be displayed with `env` or individually with `echo $VARIABLE_NAME`.
...
Bash supports comparing values between double comparison brackets, `[[ expression ]]`. It is important to keep spaces between the double brackets and the expression. These return `0` if the comparison inside the brackets is true, and `1` otherwise. The syntax is as follows:
...
For *string* comparison, Bash supports the usual comparators and a few others: 
...
Standard 'double' logical operators include `&&`, `||` and `!` as in `[[ ! $a ]]`. Also, 
...
# hello, world (planet)
```
2. strong (single) quotes where nothing is interpreted within the single quote.
```bash
echo '$greeting, world \(planet\)'
# $greeting, world \(planet\)
```
3. weak (double) quotes, where variables are interpreted, special characters are not.
```bash
echo "$greeting, world (planet)"
# hello, world
```
4. back ticks, execute and replace the output of the command with the result (AKA command substitution). For example, ```cd `pbpaste` ``` changes directory to that which is stored on the clipboard and accessed with the `pbpaste` command.
